...
Two modules have a main class: `mcp-integration` (`McpClientApplication`, the agent **server** on port 8080) and `agent-discord` (`AgentDiscordApplication`, the Discord **client**). `agent-core` is the shared logic library. Start `mcp-integration` first when you want the HTTP server available.
...
- `llm.reasoning-effort` controls the reasoning level sent to the LLM. It defaults to `medium` and is exposed locally as `LLM_REASONING_EFFORT` in `~/.hugin/hugin.env`.
...
- **`mcp-integration`** — the runnable Spring Boot **server**. It now owns the MCP server registry directly, wires `agent-core` into the MCP SDK, provides `RedisMemoryStore` (the Redis-backed `MemoryStore` impl), owns security config, and exposes the agent over HTTP (`/api/agent/chat` and the streaming `/api/agent/stream`).
...
- In-process tools the agent can call directly, with **no MCP-SDK dependency** so they live in `agent-core`: `read_file`, `write_file`, `edit_file`, `list_files`, `find_files`, `find_path`, `grep_search`, `run_bash`, `read_discord_channel`, and (when `memory.enabled`) `recall_memory`. Each implements `LocalTool` (discovered as Spring beans) and is collected by `LocalToolRegistry`. `recall_memory` is gated on `memory.enabled=true` (it depends on `MemoryService`) and lets the agent search long-term memory on demand; `read_discord_channel` surfaces up to the last 10 messages of the originating Discord channel from `ToolContext.channelMessages()` (the snapshot the bot sends on `AgentRequest.recentMessages`). `find_path` is the forgiving "locate" tool — it finds files and directories by name or path fragment (case-insensitive, fuzzy) even when the exact path doesn't exist, so the agent can resolve approximate paths the user gives (e.g. "look for the folder /code/hugin/hugin"). `find_files` (glob) takes an optional `type` (`file`/`dir`/`any`) to match directories too. Workspace-local just-in-time tools live under `.hugin/jit-tools/*.json`; Hugin rescans that directory on every agent loop iteration, so if the agent writes a new manifest and supporting script it can use the new tool on the next turn without restarting the service.
...
- The intended workspace root for local work is `~/.hugin/workspace` in an installed environment.
- The tool's `execute(args, ctx)` receives a `ToolContext` carrying the request's `Workspace`, `sessionId` (the origin, e.g. `discord-channel-123`), **and `channelMessages`** (the caller-supplied recent-message snapshot from `AgentRequest.recentMessages`, used by `read_discord_channel`). Tools that need to route something back to the caller use `ctx.sessionId()` as the reply-to target — see the scheduling tools below.
...
- **Discord short-term memory** (`agent-discord`): rather than server-side conversation memory, the bot makes the **channel itself** the short-term context. On each triggering message it fetches the recent channel history, injects only the **2 messages preceding** the one Hugin is replying to into the prompt, and forwards up to the last 10 (on `AgentRequest.recentMessages`) so the agent can pull more back with `read_discord_channel`. By default the bot only responds in guild channels when **@mentioned or replied to** (`discord.mention-only`, now defaulting to `true`); DMs are unaffected.
...
- `agent.tools.*` — built-in local tools: `enabled` (master switch), `workspace-root` (sandbox + shell working dir), `bash-timeout`, `max-output-chars` (per-result cap), `shell` (binary `run_bash` invokes; blank auto-detects `$SHELL`, falling back to `/bin/sh`), `login-shell` (default true; runs commands through a login shell `-l` so the user's profile is sourced — this is what makes Homebrew tools like `brew` on PATH resolvable).
  - `jit-tool-directory` — workspace-relative directory scanned for local JSON tool manifests. Default `.hugin/jit-tools`. Each manifest can point at a script or binary in the workspace and is reloaded on every agent loop iteration.
