...
mvn -pl mcp-integration test # test the integration module
mvn -pl mcp-client test      # test the MCP client module
...
Three modules: `mcp-integration` (`McpClientApplication`, the agent **server** on port 8080) is the runnable Spring Boot app. `agent-core` and `mcp-client` are libraries.

### Runtime prerequisites
- **An OpenAI-schema LLM endpoint** with a tool-calling model, selected via `llm.provider` in `mcp-integration/src/main/resources/application.yml`. Default is OpenRouter at `https://openrouter.ai/api/v1` (needs `OPEN_ROUTER_API_KEY`).
...
- **`mcp-client`** — the MCP server registry. Owns the full lifecycle (connect/disconnect/reconnect) of MCP servers via the MCP Java SDK, plus a `/api/v1/servers` CRUD REST API. No main class.
- **`mcp-integration`** — the runnable Spring Boot **server**. Wires the other two together. `McpToolProviderImpl` is **the only class that imports both MCP SDK types and agent-core types** — it adapts `McpServerRegistryService` to the `McpToolProvider` interface. Also provides `RedisMemoryStore` (the Redis-backed `MemoryStore` impl), owns security config, and exposes the agent over HTTP (`/api/v1/agent/chat` and the streaming `/api/v1/agent/stream`) and the cloud-agent API (`/api/v1/agents`).
...
### Streaming (`AgentService.chatStream` / `/api/v1/agent/stream`)
`chatStream` runs the exact same loop as `chat` but calls `OpenAiClient.chatStream` (`stream: true`, parses the OpenAI SSE response, reassembling content and tool-call argument fragments) and reports progress through an `AgentStreamListener` (`onContent` per text token, `onToolCall`/`onToolResult` per tool). The server's `/api/v1/agent/stream` endpoint runs the loop on a background thread (`agentStreamExecutor`) and re-emits these as SSE events — `token` `{"text"}`, `tool` `{"name","args"}`, `tool_result` `{"name","result"}`, `done`, `error`.

### Cloud agent lifecycle
`CloudAgentService` orchestrates cloud agents: allocates an agent ID, clones a repo via `RepositoryProvisioner`, registers the cloned tree as the session workspace, runs the agent loop, persists status, and reports progress via SSE.

### PR workflow
After the agent loop finishes, `CloudAgentService.run()` detects changes (`git status --porcelain`), commits with a generated message, pushes the branch, and opens a PR via the GitHub REST API. The PR URL is surfaced in the `pr_opened` SSE event and stored in `AgentInfo`.
...
- `agent.api-key` — if set, `/api/v1/**` requires the `X-API-Key` header; if blank, those endpoints are open (relies on network-level security).
...
- `agent.cloud.*` — cloud agent feature: `enabled`, `max-concurrent`, `github-token`, `branch-prefix`, `cleanup-on-complete`.
